...
public interface TemplateManager extends ModuleManager {
	...
	/**
	 * Attempts to locate a resource under the given template.  If that template does not exist, or the page does not
	 * exist under that
	 * template, will attempt to locate a similarly named file under the default template.
	 * <p>
	 * Even though the name suggests only JSP files can be located, but in fact this method can find also other
	 * resources than JSP files.
	 *
	 * @param pageContext The JSP PageContext
	...
	/**
	 * Attempts to locate a resource under the given template.  This matches the functionality findJSP(), but uses the
	 * WikiContext as
	...
	/**
	 * Lists the skins available under this template.  Returns an empty Set, if there are no extra skins available.
	 * Note that
	 * this method does not check whether there is anything actually in the directories, it just lists them.  This may
	 * change
	 * in the future.
	 *
	 * @param pageContext the JSP PageContext
	...
	/**
	 * Extract all i18n strings in the javascript domain. (javascript.*) Returns a javascript snippet which defines the
	 * LocalizedStings array.
	 *
	 ...
	 * <p>
	 * The resources can be of different types.  For RESOURCE_SCRIPT and RESOURCE_STYLESHEET this is an URI path to the
	 * resource
	 * (a script file or an external stylesheet) that needs to be included.  For RESOURCE_INLINECSS the resource should
	 * be something
	 * that can be added between &lt;style>&lt;/style> in the header file (commonheader.jsp).  For RESOURCE_JSFUNCTION
	 * it is the name
	 * of the Javascript function that should be run at page load.
	 * <p>
	 * The IncludeResourceTag inserts code in the template files, which is then filled by the WikiFilter after the
	 * request has been
	 * rendered but not yet sent to the recipient.
	 * <p>
	 * Note that ALL resource requests get rendered, so this method does not check if the request already exists in the
	 * resources.
	 * Therefore, if you have a plugin which makes a new resource request every time, you'll end up with multiple
	 * resource requests
