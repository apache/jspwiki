...
/**
 * Translates to JSPWiki syntax from a plain text handling italic.
 */
class PlainTextItalicDecorator {

    final PrintWriter out;
    final Stack< String > preStack;
    final XHtmlElementToWikiTranslator chain;
    final PlainTextMonospaceDecorator ptmd;

    PlainTextItalicDecorator( final PrintWriter out, final Stack< String > preStack, final XHtmlElementToWikiTranslator chain ) {
        this.out = out;
        this.preStack = preStack;
        this.chain = chain;
        this.ptmd = new PlainTextMonospaceDecorator( out, preStack, chain );
    }
