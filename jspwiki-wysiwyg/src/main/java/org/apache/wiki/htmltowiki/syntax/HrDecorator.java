...
/**
 * Translates to wiki syntax from an {@code HR} element.
 */
public abstract class HrDecorator {
    ...
    protected HrDecorator( final PrintWriter out, final XHtmlElementToWikiTranslator chain ) {
        this.out = out;
        this.chain = chain;
    }
    ...
    public void decorate( final Element e ) throws JDOMException {
        out.println();
        MarkupHelper.printUnescaped( out, markupHr() );
        chain.translate( e );
        out.println();
    }

    /**
     * Wiki markup for a {@code HR} element.
     *
     * @return Wiki markup for a {@code HR} element.
     */
