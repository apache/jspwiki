...
JSSpec.Logger.prototype.onRunnerStart = function() {
	...
	list.innerHTML = [
		...
		function() {
			...
			for(var i = 0; i < specs.length; i++) {
				var spec = specs[i];
				sb.push('<li id="spec_' + specs[i].id + '_list"><h3><a href="#spec_' + specs[i].id + '">' + JSSpec.util.escapeTags(specs[i].context) + '</a> [<a href="?rerun=' + encodeURIComponent(specs[i].context) + '">rerun '+ specs[i].examples.length + '</a>]</h3> </li>');
			}
			return sb.join("");
		}(),
		'</ul>'
	...
	log.innerHTML = [
		...
		function() {
			...
			for(var i = 0; i < specs.length; i++) {
				...
				sb.push('		<h3>' + JSSpec.util.escapeTags(specs[i].context) + ' [<a href="?rerun=' + encodeURIComponent(specs[i].context) + '">rerun '+ specs[i].examples.length + '</a>]</h3>');
