...
public class DecisionQueueTest extends TestCase
{
    ...
    protected void setUp() throws Exception
    {
        ...
        d1 = new SimpleDecision(w, "decision1.key", new WikiPrincipal("Owner1"));
        d2 = new SimpleDecision(w, "decision2.key", new WikiPrincipal("Owner2"));
        d3 = new SimpleDecision(w, "decision3.key", new WikiPrincipal("Owner3"));
        m_queue.add(d1);
        m_queue.add(d2);
        m_queue.add(d3);
    }
    ...
    public void testComplete() throws WikiException
    {
        ...
        m_queue.decide(d2, Outcome.DECISION_HOLD);
    ...
    public void testReassign() throws WikiException
    {
        ...
        // Owner3 owns 1 decision (d3)
        assertEquals(new WikiPrincipal("Owner3"), d3.getActor());
        assertEquals(1, m_queue.actorDecisions(new WikiPrincipal("Owner3")).length);
        ...
        // Owner3's decision list
        assertEquals(new WikiPrincipal("NewOwner"), d3.getActor());
        assertEquals(0, m_queue.actorDecisions(new WikiPrincipal("Owner3")).length);
    }
    ...
    public void testOwnerDecisions()
    {
        Decision[] decisions = m_queue.actorDecisions(new WikiPrincipal("Owner1"));
        assertEquals(1, decisions.length);

        decisions = m_queue.actorDecisions(new WikiPrincipal("Owner2"));
        assertEquals(1, decisions.length);

        decisions = m_queue.actorDecisions(new WikiPrincipal("Owner3"));
        assertEquals(1, decisions.length);
    }
    ...
    public void testDecisionWorkflow() throws WikiException
    {
        ...
        Decision decision = new SimpleDecision(w, "decision.Actor1Decision", new WikiPrincipal("Actor1"));
        ...
        // Verify that it's also in the DecisionQueue
        Decision[] decisions = m_queue.actorDecisions(new WikiPrincipal("Actor1"));
        assertEquals(1, decisions.length);
        assertEquals(decision, decisions[0]);
        
        // Make Decision, and verify that it's gone from the queue
        m_queue.decide(decision, Outcome.DECISION_APPROVE);
        decisions = m_queue.actorDecisions(new WikiPrincipal("Actor1"));
        assertEquals(0, decisions.length);
    }
