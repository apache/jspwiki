...
/*
 * chartist behavior...
 * hope this is the right pattern... see JSPWIKI-1102
 * 
 * a jspwiki "behavior" to search for %%chart usages, then do the find
 * the next table, read the data, parse it, then convert the whole thing
 * to a chartist based chart, (line, bar or pie charts)
 * more info at chartit-js. https://github.com/chartist-js/chartist
 * 
 * Requires the ChartistPlugin to be on the page first to load the chartist
 * css and js files. If you're looking for those, they are loaded via webjars
 */
    ...
    */
    ...
    chartist_behavior: function (element) {
        ...
        if (type.match(/Line|Bar|Pie/)) {
            ...
            if (!pageContent) {
                pageContent.parentElement.parentElement;
